...
const TimelineStep: React.FC<TimelineStepProps> = ({ title, description, icon, index }) => (
  <div className={`flex flex-col md:flex-row items-center gap-8 mb-24 md:mb-32 ${index % 2 === 0 ? 'md:flex-row' : 'md:flex-row-reverse'}`}>
    ...
    <div className="hidden md:block w-1/2">
      <div className="aspect-square glass-card rounded-3xl flex items-center justify-center p-8 transition-all hover:bg-white/60">
        <img 
           src={`https://picsum.photos/seed/love-${index}/600/600`} 
           alt={title} 
           className="rounded-2xl shadow-lg opacity-90 transition-opacity hover:opacity-100 grayscale-[20%] hover:grayscale-0"
        />
      </div>
    </div>
  </div>
